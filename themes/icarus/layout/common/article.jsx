        ...
        return <Fragment>
            ...
            <div class="card">
                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`} role="article">
                    {/* Title (show first on index) */}
                    {page.title !== '' && index ? <div class="title-div"><p class="title is-2 is-size-2-mobile"><a class="link-muted" href={url_for(page.link || page.path)}>{page.title}</a></p></div> : null}
                    {page.title !== '' && !index ? <h1 class="title is-2 is-size-3-mobile" style="margin-bottom: 0.25rem !important;">{page.title}</h1> : null}
                    {/* Metadata */}
                    {page.layout !== 'page' ? <div class="article-meta is-size-7 is-uppercase level is-mobile">
                        <div class={index ? "level-left ml-1" : "level-left"}>
                            {/* Creation Date */}
                            {page.date ? <span class="level-item">{date(page.date, 'YYYY-MM-DD HH:mm')}&nbsp;</span> : null}
                            {/* Categories */}
                            {page.categories && page.categories.length ? <span class="level-item">
                                {(() => {
                                    const categories = [];
                                    page.categories.forEach((category, i) => {
                                        categories.push(<a class="link-muted category-link" href={url_for(category.path)}>{category.name}</a>);
                    ...
                    {/* Content/Excerpt */}
                    <div class={`content${index && page.excerpt ? ' preview-text' : ''}`} dangerouslySetInnerHTML={{ __html: index && page.excerpt ? page.excerpt : page.content }}></div>
                    {/* Licensing block */}
